...
/*
an observable is a very simple idea, it is a `function` that can be called in 3 ways,
and represents a mutable value.

If an observable is called with no arguments, it returns the current value.
If it is returned with an argument, it set that as the value.
And, if a function is called with another function, it _calls_ that function with
the new value, whenever the value changes.

you can write a custom observable is just a few lines.
*/
...
function not (observable) {
  return function (val) {
    return (
      get(val) ? !(observable())
    : set(val) ? observable(!val)
    : observable(function (_val) { val(!val) })
    )}}
...
function property (model, key) {
  return function (value) {
    ...
    return function () {
      //work with node.js and backbone
...
/*
hang on, lets just refactor this a little bit...

some functions to check if a value signifies a get, or a set
(if both these tests fail, we know it's a listen)

### get(val) _(private)_
return true if this call is a get (no arg is supplied)
*/
...
require.define("/example/tmp.js",function(require,module,exports,__dirname,__filename,process,global){var funx = [
function () {
...
<p>okay ^ that is running on the server...


</p>
<pre><code>var o = require(&#39;observable&#39;)()
o(10)
o()</code></pre>
<p>okay!

</p>
<pre><code>var h = require(&#39;h&#39;)

h(&#39;h2&#39;, &quot;it&#39;s html inside js&quot;)</code></pre>
<p>combined
